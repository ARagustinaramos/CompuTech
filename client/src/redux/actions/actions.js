...
export const getCategories = () => async (dispatch) => {
	try {
		const response = await fetch(
			"https://computechback.onrender.com/categories"
		);
		const data = await response.json();
		...
		try {
			...
			if (name.trim() === "") {
				...
				response = await axios.get(
					"https://computechback.onrender.com/products"
				);
			} else {
				// Busca productos por nombre
				response = await axios.get(
					`https://computechback.onrender.com/products/name?name=${name}`
				);
			}
	...
	return async (dispatch) => {
		try {
			const { data } = await axios.get(
				`https://computechback.onrender.com/products/${id}`
			);
			return dispatch({
				type: GET_DETAIL,
				payload: data
...
export const deleteProduct = (id, boolean) => {
	return async (dispatch) => {
		try {
			...
			await axios.delete(
				`https://computechback.onrender.com/products/delete/${id}`,
				{
	...
	return async (dispatch) => {
		try {
			...
			const response = await axios.delete(
				`https://computechback.onrender.com/users/${email}`
			);
...
export const getAllProducts = () => {
	return async (dispatch) => {
		try {
			const { data } = await axios.get(
				"https://computechback.onrender.com/products/all"
			);
			return dispatch({
...
export const getAllSales = () => {
	return async (dispatch) => {
		try {
			const { data } = await axios.get(
				"https://computechback.onrender.com/order/"
			);
			return dispatch({
				type: GET_SALES,
				payload: data
...
export const getAllReviews = () => async (dispatch) => {
	try {
		const response = await axios.get(
			"https://computechback.onrender.com/reviews"
		);
		dispatch({
	...
	return async (dispatch) => {
		try {
			const { data } = await axios.put(
				`https://computechback.onrender.com/users/put/${id}`,
				{
					name: userData.name,
