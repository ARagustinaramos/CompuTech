...
import { addToCart, createReview } from '../../redux/actions/actions';
import Swal from 'sweetalert2';
import { useAuthState } from 'react-firebase-hooks/auth';
import { auth } from '../../firebase/firebase';
import ComponentReview from '../../views/dashboard/user/components/ComponentReview';
...
const Card = ({ id_Product, name, image, price }) => {
  ...
  const handleAddReview = (reviewData) => {
    if (!user) {
      Swal.fire("Por favor inicia sesión para dejar tu review");
      return;
    }
    dispatch(createReview(reviewData));
    Swal.fire("Gracias por comentar!");
  };
      ...
      <div className="px-5 pb-5">
        ...
        <div className="flex items-center mt-2.5 mb-5">
          <div className="flex items-end">
            ...
            <span className="text-gray-600 pl-0 ml-3">{`${4} Reviews`}</span>
          </div>
            {
            user ? (
            <button
              onClick={openModalReview}
              className="text-white ml-20 bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-1.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"
            >
              Opinar
            </button>) :
            null
