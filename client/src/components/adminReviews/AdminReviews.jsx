...
const AdminReviews = () => {
	...
	useEffect(() => {
		// Fetch all reviews
		axios
			.get("https://computechback.onrender.com/reviews")
			.then((response) => {
				setReviews(response.data);
				// Filter reviews with status 'pending'
				setPendingReviews(
					response.data.filter((review) => review.status === "pending")
				);
			})
	...
	const handleApprove = async (reviewId) => {
		try {
			await axios.put(
				`https://computechback.onrender.com/reviews/${reviewId}`,
				{ status: "approved" }
			);
			...
			setPendingReviews((prevReviews) =>
				prevReviews.filter((review) => review.id_Review !== reviewId)
			);
		} catch (error) {
		...
		for (let i = 1; i <= 5; i++) {
			stars.push(
				<svg
					key={i}
					className={`w-5 h-5 ${
						i <= ranking ? "text-yellow-400" : "text-gray-300"
					}`}
							...
							{pendingReviews.length === 0 ? (
								...
								pendingReviews.map((review) => (
									<div
										...
										className="bg-white p-6 rounded-lg shadow mb-4 dark:bg-gray-800 dark:border-gray-700"
									>
										<div className="mb-2">
											<p className="text-gray-900 dark:text-gray-100">
												<strong>Usuario:</strong> {review.User.name}
											</p>
											<p className="text-gray-900 dark:text-gray-100">
												<strong>Producto:</strong> {review.Product.name}
											</p>
										</div>
										<div className="mb-2">{renderStars(review.ranking)}</div>
										<p className="text-gray-800 dark:text-gray-200">
