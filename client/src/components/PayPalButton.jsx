...
const PayPalButton = ({ total, items }) => {
	...
	const handleOrderComplete = async (detail, userId) => {
		...
		if (detail.status === "COMPLETED") {
			...
			try {
				await axios.post(
					`https://computechback.onrender.com/order/create-order/${userId}`,
					formattedOrder
				);
				...
				for (const item of items) {
					...
					const productResponse = await axios.get(
						`https://computechback.onrender.com/products/${item.id_Product}`
					);
					...
					if (response.status !== 200) {
						throw new Error(
							`No se pudo actualizar el stock para el producto ${item.name}`
						);
					}
				}
				...
				console.error(
					`Error al enviar los datos del pedido: ${userId}`,
