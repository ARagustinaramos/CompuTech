...
const Cart = () => {
	...
	useEffect(() => {
		if (user && user.email) {
			fetch("https://computechback.onrender.com/users")
		...
		if (isAuthenticated && userId) {
			try {
				...
				if (!updateCartResponse.ok) {
					throw new Error(
						"No se pudo actualizar los ítems del carrito del usuario"
					);
				}
	...
	const handleSaveOrder = async (e) => {
		...
		try {
			const response = await fetch(
				"https://computechback.onrender.com/api/create-order",
				{
					method: "POST",
					headers: {
						"Content-Type": "application/json"
					...
					<div className="mx-auto max-w-screen-xl px-4 2xl:px-0">
						...
						{cartItems?.length === 0 || !isAuthenticated ? (
							<div className="flex flex-col items-center justify-center mt-6 sm:mt-8">
								<img
									src="https://www.ancestralanimalsoul.com/imagenes/carrito-vacio.png"
									alt="Carrito vacío"
								...
								<Link
									to="/"
									className="mt-4 inline-block rounded-lg bg-blue-500 px-5 py-2.5 text-sm font-medium text-white hover:bg-blue-600 focus:outline-none focus:ring-4 focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"
								>
									Seguir comprando
								</Link>
							</div>
														...
														<div className="flex items-center gap-4">
															...
															<div className="flex h-4 items-center border-l border-gray-300 dark:border-gray-600">
																<button
																	...
																	onClick={() =>
																		handleRemoveItemClick(item?.cartItemId)
																	}
																>
																	Eliminar
																</button>
